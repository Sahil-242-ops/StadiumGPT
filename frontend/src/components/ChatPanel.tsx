...
const I18N: Record<string, {
  ...
  en: {
    placeholder: "Ask me anything… e.g. 'Nearest step-free route to Section B2'",
  ...
  es: {
    ...
    welcomeHint: '¡Intenta hacer clic en una sugerencia a continuación o haz una pregunta!',
  },
  fr: {
    ...
    welcomeSub: 'Je suis votre guide IA pour la Coupe du Monde de la FIFA 2026.',
    ...
    welcomeHint: 'Essayez de cliquer sur une suggestion ci-dessous ou de poser une question !',
  },
  hi: {
    ...
    welcomeHint: 'नीचे दिए गए सुझाव पर क्लिक करने का प्रयास करें या प्रश्न पूछें!',
  },
};
...
export default function ChatPanel({ stadiumId, language, accessibilityMode, externalQuery, onQueryConsumed }: ChatPanelProps) {
  const t = I18N[language] ?? I18N.en;

  const [messages, setMessages] = useState<ChatMessage[]>([
    { id: 'welcome', role: 'bot', text: '__welcome__', timestamp: new Date() },
  ]);
  const [suggestions, setSuggestions] = useState<string[]>(SUGGESTED.general ?? []);
  ...
  const send = useCallback(
    async (message: string) => {
      ...
      const userMsg: ChatMessage = { id: uid(), role: 'user', text: message, timestamp: new Date() };
      const thinkingId = uid();
      setMessages((prev) => [...prev, userMsg, { id: thinkingId, role: 'bot', text: '__typing__', timestamp: new Date() }]);
      setInput('');
      setLoading(true);

      try {
        const data = await sendChat({ message, language, stadium_id: stadiumId, accessibility_mode: accessibilityMode });
        setMessages((prev) =>
          prev.map((m) =>
            m.id === thinkingId
  ...
  return (
    <section className="panel panel--chat" aria-labelledby="chat-heading">
      <h1 id="chat-heading" className="sr-only">StadiumGPT AI Chat Assistant</h1>
              ...
              <div className="msg__bubble">
                {isTyping ? (
                  ...
                  <>
                    ...
                    <p className="msg__text" style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                      {t.welcomeHint}
                    </p>
                  </>
                ) : (
      ...
      >
        ...
        <button type="submit" className="btn btn--primary" aria-label="Send message" disabled={loading}>
