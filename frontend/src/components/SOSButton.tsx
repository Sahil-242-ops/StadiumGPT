...
const SOS_OPTIONS: { type: SOSType; icon: string; label: string; color: string }[] = [
  { type: 'medical',    icon: '🚑', label: 'Medical Emergency',  color: 'var(--accent-red)' },
  { type: 'lost_child', icon: '👶', label: 'Lost Child',         color: 'var(--accent-orange)' },
  { type: 'security',   icon: '🛡️', label: 'Security Threat',    color: 'var(--accent-purple)' },
  { type: 'fire',       icon: '🔥', label: 'Fire / Evacuation',  color: 'var(--fifa-red)' },
  { type: 'general',    icon: '❓', label: 'Other Emergency',    color: 'var(--accent-primary)' },
];
...
export default function SOSButton() {
  ...
  return (
    <>
      ...
      {/* SOS Modal */}
      {open && (
        <div className="sos-overlay" role="dialog" aria-modal="true" aria-label="Emergency SOS">
          <div className="sos-modal">
            <button className="sos-modal__close" onClick={close} aria-label="Close">✕</button>
            ...
            {!result ? (
              <>
                <div className="sos-modal__header">
                  ...
                  <p className="sos-modal__subtitle">Select the type of emergency</p>
                </div>
                ...
                <div className="sos-modal__options">
                  {SOS_OPTIONS.map((opt) => (
                    <button
                      ...
                      disabled={loading}
                      style={{ '--sos-color': opt.color } as React.CSSProperties}
                    >
              ...
              <div className="sos-result">
                ...
                <div className="sos-result__details">
                  <div className="sos-result__row">
                    ...
                    <span className="sos-result__value">{result.responder}</span>
                  </div>
                  <div className="sos-result__row">
                    <span className="sos-result__label">ETA</span>
                    <span className="sos-result__value sos-result__eta">{result.eta_minutes} min</span>
                  </div>
                  <div className="sos-result__row">
                  ...
                  <div className="sos-result__row">
                    ...
                    <span className="sos-result__value" style={{ fontFamily: 'monospace', fontSize: '0.75rem' }}>{result.id}</span>
                  </div>
                </div>
