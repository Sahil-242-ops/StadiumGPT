...
describe('POST /api/chat', () => {
  ...
  it('handles accessibility_mode flag', async () => {
    const res = await request(app)
      .post('/api/chat')
      .send({ message: 'Find me a wheelchair route', accessibility_mode: true });
    expect(res.status).toBe(200);
...
describe('POST /api/crowd', () => {
  ...
  it('overall_level is a valid CrowdLevel', async () => {
    ...
    expect(res.status).toBe(200);
    expect(['low', 'moderate', 'high', 'critical']).toContain(res.body.overall_level);
  });
...
describe('POST /api/sos', () => {
  const validTypes = ['medical', 'lost_child', 'security', 'fire', 'general'] as const;
