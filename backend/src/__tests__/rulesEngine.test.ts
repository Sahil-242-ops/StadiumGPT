...
describe("detectIntent", () => {
  ...
  it("detects accessibility intent", () => {
    expect(detectIntent("wheelchair ramp near the entrance")).toBe(
      "accessibility",
    );
    expect(detectIntent("Is there a step-free entrance option?")).toBe(
      "accessibility",
    );
  ...
  it("detects emergency intent", () => {
    ...
    expect(detectIntent("call the police there is an attack")).toBe(
      "emergency",
    );
  ...
  it("detects sustainability intent", () => {
    expect(detectIntent("where are the recycling bins?")).toBe(
      "sustainability",
    );
...
describe("resolve — navigate intent", () => {
  ...
  it("returns step-free route when step_free=true", () => {
    const result = resolve(
      "navigate to section A1 via gate A",
      "met_life",
      "A1",
      true,
    );
...
describe("resolve — crowd intent", () => {
  ...
  it("each zone has required fields", () => {
    ...
    const zones = (result.facts["zones"] ?? result.facts) as Record<
      string,
      unknown
    >[];
    expect(Array.isArray(zones)).toBe(true);
