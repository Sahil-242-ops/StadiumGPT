...
function getStadiums() {
  if (!_stadiums)
    _stadiums = loadJson<Record<string, unknown>>("stadiums.json");
...
function resolveNavigation(
  ...
  const sectionMatch = message
    .toUpperCase()
    .match(/SECTION\s+([A-C][1-3]|ACCESSIBLE|VIP|PRESS)/);
  ...
  if (!route) {
    ...
    const levelNum = section.startsWith("A")
      ? "1"
      : section.startsWith("B")
        ? "2"
        : "3";
    const estimated_minutes =
      Math.floor(seededRand(`${gate}${section}min`) * 5) + 4;
    const distance_metres =
      estimated_minutes * 60 +
      Math.floor(seededRand(`${gate}${section}dist`) * 40);
    ...
    route = {
      ...
      estimated_minutes,
      landmarks: [
        `Gate ${gate} Information board`,
        `Level ${levelNum} junction`,
        `Section ${section} entryway`,
      ],
    };
  }
...
/**
 * Simulates and resolves stand/zone occupancy levels in a deterministic manner.
 * Seeded by the current hour to ensure output stability across requests.
 *
 * @param stadiumId Unique identifier for the target stadium.
 * @param section Optional stadium seat section code.
 * @returns Object holding overall crowd safety state and standings zones details.
 */
function resolveCrowd(
  ...
  const zones = [
    ...
    return {
      ...
      ...(level === "critical"
        ? { alert: "High congestion — use Gate D or C" }
        : {}),
    };
  });
...
function resolveFacility(message: string): Record<string, unknown> {
  ...
  if (
    ...
    return {
      ...
      operating_hours:
        "Match day: 4 hours before kick-off to 1 hour after final whistle",
    };
  }
...
/**
 * Primary facts resolver inside the rules engine.
 * Receives the text message query and maps it to a specific intent category
 * with pre-resolved structured facts.
 *
 * @param message The raw user query.
 * @param stadiumId Target stadium identifier (defaults to 'met_life').
 * @param section Optional target stand/seat section.
 * @param stepFree Whether step-free/accessible routing is requested.
 * @returns The resolved facts and categorization wrapper.
 */
export function resolve(
  ...
  if (detectInjection(message)) {
    return {
      intent: "general",
      facts: {
        info: "I can only assist with stadium navigation and fan services.",
      },
      urgency: "normal",
    };
  }
  ...
  switch (intent) {
    ...
    case "crowd":
      return {
        intent,
        facts: resolveCrowd(stadiumId, section) as Record<string, unknown>,
        urgency: "normal",
      };
    case "facility":
    ...
    default:
      return {
        intent: "general",
        facts: {
          info: "Welcome to StadiumGPT! Ask me about navigation, facilities, crowd levels, or transport.",
        },
        urgency: "normal",
      };
  }
